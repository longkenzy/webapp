...
export default function CreateReceivingCaseModal({ isOpen, onClose, onSuccess, editData }: CreateReceivingCaseModalProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (employee) {
          ...
          console.log('✅ Current employee found:', employee.fullName);
        } else {
    ...
    if (products.length === 0) {
      ...
      products.forEach((product, index) => {
        ...
        if (!product.quantity.trim()) {
          ...
          console.log(`❌ Product ${index} missing quantity`);
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('🔍 End Date being sent:', formData.completionDateTime);
        ...
        <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 md:px-6 py-3 md:py-4 rounded-t-2xl md:rounded-t-lg z-40 flex-shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <div>
                <h2 className="text-base md:text-lg font-semibold">{editData ? 'Chỉnh sửa Case Nhận Hàng (Admin)' : 'Tạo Case Nhận Hàng (Admin)'}</h2>
                <p className="text-blue-100 text-xs md:text-sm hidden sm:block">{editData ? 'Cập nhật thông tin nhận hàng' : 'Hệ thống quản lý nhận hàng'}</p>
              </div>
            </div>
            <button
              onClick={handleClose}
          ...
          <div className="space-y-4 md:space-y-6">
            ...
            <div className="bg-gray-50 rounded-md p-3 md:p-4 border border-gray-200">
              <div className="flex items-center gap-2 mb-3 md:mb-4">
                ...
                <h3 className="text-xs md:text-sm font-semibold text-gray-700">Người nhận hàng</h3>
              </div>
