...
import { X, Shield, Calendar, FileText, RefreshCw, Save } from 'lucide-react';
  ...
  useEffect(() => {
    if (isOpen && warrantyData) {
      setFormData({
        ...
        status: warrantyData.status || 'RECEIVED',
        ...
        <div className="ios-input-fix bg-white rounded shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
         ...
         <div className="bg-gradient-to-r from-emerald-600 to-teal-600 px-5 py-4 flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-white" style={{ fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>Chỉnh sửa Case Bảo Hành</h2>
                <p className="text-emerald-50 text-xs mt-0.5">Cập nhật thông tin case bảo hành</p>
              </div>
            </div>
            <button
              type="button"
              ...
              <div className="bg-white rounded border border-gray-200">
                ...
                <div className="p-3 space-y-2 text-sm">
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-gray-900 flex-1">{warrantyData.title}</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="font-medium text-gray-600 min-w-[120px]">Loại bảo hành:</span>
                    <span className="text-gray-900 flex-1">{formatWarrantyType(warrantyData.warrantyType)}</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="font-medium text-gray-600 min-w-[120px]">Người xử lý:</span>
                    <span className="text-gray-900 flex-1">{warrantyData.handler ? warrantyData.handler.fullName : 'Chưa xác định'}</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="font-medium text-gray-600 min-w-[120px]">Ngày bắt đầu:</span>
                    <span className="text-gray-900 flex-1">{formatVietnamDateTime(warrantyData.startDate)}</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="font-medium text-gray-600 min-w-[120px]">Khách hàng:</span>
                    <span className="text-gray-900 flex-1">{warrantyData.customerName || 'Chưa có'}</span>
                  </div>
                  <div className="flex items-start gap-2">
                ...
                <div className="p-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <DateTimePicker
                      ...
                      clearable
                      minDate={new Date(warrantyData.startDate)}
                      withSeconds={false}
                      styles={{
                        input: {
                          fontSize: '0.875rem',
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    <span>Cập nhật Case Bảo Hành</span>
                  </>
                )}
              </button>
            </div>
