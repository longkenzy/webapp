...
import { Plus, Settings, Shield, FileText, Calendar, Zap, Search, RefreshCw, Eye, Edit, Trash, AlertTriangle, CheckCircle, Download, X, Filter, ChevronDown } from 'lucide-react';
...
export default function AdminWarrantyWorkPage() {
  ...
  const fetchWarranties = useCallback(async (retryCount = 0) => {
    try {
      ...
      const response = await fetch('/api/warranties?limit=1000', {
        method: 'GET',
        headers: {
  ...
  const filteredWarranties = useMemo(() => warranties.filter(warranty => {
    const matchesSearch = !debouncedSearchTerm ||
      warranty.title.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
      warranty.description.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
      warranty.customerName.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
      warranty.handler.fullName.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
      (typeof warranty.warrantyType === 'string' ? warranty.warrantyType : warranty.warrantyType?.name || '').toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
      (warranty.customer?.fullCompanyName.toLowerCase().includes(debouncedSearchTerm.toLowerCase())) ||
      (warranty.customer?.shortName.toLowerCase().includes(debouncedSearchTerm.toLowerCase()));
    ...
    const matchesDateFrom = !dateFrom || new Date(warranty.startDate) >= dateFrom;
    const matchesDateTo = !dateTo || new Date(warranty.startDate) <= dateTo;

    return matchesSearch && matchesHandler && matchesStatus && matchesWarrantyType &&
      matchesCustomer && matchesDateFrom && matchesDateTo;
  }).sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime()), [warranties, debouncedSearchTerm, selectedHandler, selectedStatus, selectedWarrantyType, selectedCustomer, dateFrom, dateTo]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [debouncedSearchTerm, selectedHandler, selectedStatus, selectedWarrantyType, selectedCustomer, dateFrom, dateTo]);
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      {(selectedHandler || selectedStatus || selectedWarrantyType || selectedCustomer || dateFrom || dateTo) && (
                        <span className="absolute -top-1 -right-1 flex items-center justify-center w-4 h-4 text-[9px] font-bold text-white bg-red-500 rounded-full">
                          {[selectedHandler, selectedStatus, selectedWarrantyType, selectedCustomer, dateFrom, dateTo].filter(Boolean).length}
                        </span>
                      )}
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {paginatedWarranties.map((warranty, index) => (
                          <React.Fragment key={warranty.id}>
                            <tr
                              className={`hover:bg-gray-50/50 transition-colors duration-150 cursor-pointer ${!isWarrantyEvaluatedByAdmin(warranty) ? 'bg-yellow-50/50 border-l-4 border-l-yellow-400' : ''
                                }`}
                              ...
                              <td className="px-2 py-4 whitespace-nowrap text-sm font-medium w-20">
                                <div className="flex items-center space-x-1">
                                  ...
                                  <button
                                    ...
                                    className={`p-1.5 rounded-md transition-colors duration-200 ${isWarrantyEvaluatedByAdmin(warranty)
