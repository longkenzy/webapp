...
export default function CreateDeliveryCaseModal({ isOpen, onClose, onSuccess, editData }: CreateDeliveryCaseModalProps) {
  ...
  const [formData, setFormData] = useState({
    customerId: '',
    handler: '', // Admin-specific: Handler ID
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // In edit mode, preserve the original requesterId
      // In create mode, use currentEmployee as requester
      const reporterId = editData ? editData.requester.id : (currentEmployee?.id || formData.handler);
        ...
        <div className="ios-input-fix bg-white rounded shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-white" style={{ fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
                  {editData ? 'Chỉnh sửa Case Giao Hàng (Admin)' : 'Tạo Case Giao Hàng (Admin)'}
                </h2>
                <p className="text-blue-50 text-xs mt-0.5">Quản lý giao hàng đến khách hàng</p>
              </div>
            </div>
            <button
              type="button"
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto bg-gray-50">
            <div className="p-5 space-y-4">
              {/* Row 1: Người giao hàng + Khách hàng */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Section 1: Người giao hàng (Admin specific) */}
                <div className="bg-white rounded border border-gray-200">
                  <div className="bg-gray-50 px-3 py-2 border-b border-gray-200 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <User className="h-4 w-4 text-blue-600" />
                      <h3 className="text-sm font-semibold text-gray-900" style={{ fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>Người giao hàng</h3>
                    </div>
