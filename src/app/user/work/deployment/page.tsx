...
import { Plus, Search, Filter, MoreVertical, Edit, RefreshCw, X, Rocket } from 'lucide-react';
...
export default function DeploymentCasePage() {
  ...
  const { deploymentCases, loading, error, refreshCases } = useDeploymentCases();
  ...
  const handleEditSuccess = async (updatedCase: DeploymentCase) => {
    // Refresh the cases to get the updated data
    await refreshCases();
  };
  ...
  const filteredCases = deploymentCases
    .filter(case_ => {
      ...
      const matchesReporter = filters.reporter === '' || 
        case_.reporter.fullName.toLowerCase().includes(filters.reporter.toLowerCase());
  ...
  const getUniqueReporters = () => {
    const reporters = deploymentCases.map(case_ => case_.reporter.fullName);
    return [...new Set(reporters)].sort();
  };

  const getUniqueHandlers = () => {
    const handlers = deploymentCases.map(case_ => case_.handler.fullName);
    return [...new Set(handlers)].sort();
  };

  const getUniqueDeploymentTypes = () => {
    const deploymentTypes = deploymentCases.map(case_ => case_.deploymentType.name);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6 pt-4">
                ...
                {loading ? (
                  ...
                  filteredCases.map((case_, index) => (
                    <tr key={case_.id} className="hover:bg-slate-50/50 transition-colors duration-150">
                      ...
                      <td className="px-3 py-2">
                        <div>
                          <div className="text-sm text-slate-900">{case_.reporter.fullName}</div>
                          <div className="text-xs text-slate-500">{case_.reporter.position}</div>
                        </div>
                      </td>
                      <td className="px-3 py-2">
                        <div>
                      ...
                      <td className="px-3 py-2">
                        <div className="text-sm text-slate-700">{case_.deploymentType.name}</div>
                        <div className="text-xs text-slate-500">{case_.customerName}</div>
                      </td>
                      <td className="px-3 py-2">
      ...
      <CreateDeploymentModal
        ...
        onSuccess={async () => {
          // Refresh cases to get the latest data including the new case
          await refreshCases();
        }}
      />
