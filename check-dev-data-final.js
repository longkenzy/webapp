...
async function checkDevData() {
    ...
    try {
        ...
        const maintenanceCaseTypeCount = await prisma.maintenanceCaseType.count();
        console.log(`MaintenanceCaseType${' '.repeat(4)}: ${maintenanceCaseTypeCount} bản ghi`);

        const warrantyTypeCount = await prisma.warrantyType.count();
        console.log(`WarrantyType${' '.repeat(9)}: ${warrantyTypeCount} bản ghi`);

        const deploymentTypeCount = await prisma.deploymentType.count();
        console.log(`DeploymentType${' '.repeat(7)}: ${deploymentTypeCount} bản ghi`);
        ...
        console.log(`Warranty${' '.repeat(13)}: ${warrantyCount} bản ghi`);

        const deploymentCaseCount = await prisma.deploymentCase.count();
        console.log(`DeploymentCase${' '.repeat(7)}: ${deploymentCaseCount} bản ghi`);
        ...
        const totalRecords = userCount + partnerCount + employeeCount + incidentTypeCount + 
                           maintenanceCaseTypeCount + warrantyTypeCount + deploymentTypeCount +
                           evaluationConfigCount + incidentCount + maintenanceCaseCount + 
                           warrantyCount + deploymentCaseCount + internalCaseCount + 
                           deliveryCaseCount + receivingCaseCount + notificationCount + scheduleCount;
        ...
        if (totalRecords === 0) {
            ...
            console.log('');
            console.log('Hoặc copy chi tiết:');
            console.log('  .\\copy-prod-data-to-dev.ps1');
        } else {
            console.log('');
