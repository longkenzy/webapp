...
import { Plus, Search, Filter, MoreVertical, Edit, RefreshCw, X, AlertTriangle, Check, ChevronDown, User, Building2, Calendar, Settings } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-red-50 p-3 md:p-6 pt-3 md:pt-4">
      ...
      <div className="max-w-full mx-auto px-2 md:px-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl md:text-3xl font-bold text-slate-900 mb-1 md:mb-2 flex items-center">
                <AlertTriangle className="h-5 w-5 md:h-8 md:w-8 text-red-600 mr-2 md:mr-3" />
                Quản Lý Sự Cố
              </h1>
              <p className="text-xs md:text-base text-slate-600 hidden md:block">Theo dõi và xử lý các sự cố bảo mật và hệ thống</p>
            </div>
            <button
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-4 md:mb-6">
          {/* Header */}
          <div className="bg-gradient-to-r from-red-50 to-orange-50 px-3 md:px-6 py-3 md:py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 md:gap-3">
                <div className="p-1.5 md:p-2 bg-red-100 rounded-lg">
                  <Search className="h-4 w-4 md:h-5 md:w-5 text-red-600" />
                </div>
                <div>
            ...
            <div className="space-y-2 md:space-y-3">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 md:pl-10 pr-2.5 md:pr-3 py-1.5 md:py-2 border border-gray-200 rounded-md focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-200 bg-gray-50 focus:bg-white text-xs md:text-sm"
                  />
                </div>
              </div>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  >
                    ...
                    {getActiveFiltersCount() > 0 && (
                      <span className="inline-flex items-center justify-center h-5 w-5 rounded-full bg-red-100 text-red-600 text-xs font-medium">
              ...
              <div className="flex items-center justify-between pt-2 md:pt-3 border-t border-gray-200">
                <div className="text-xs md:text-sm text-gray-600">
                  <span className="font-medium text-gray-900">{filteredIncidents.length}</span> / <span className="font-medium text-gray-900">{incidents.length}</span> case
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="md:hidden space-y-3 mb-4">
          {loading ? (
            ...
            paginatedIncidents.map((incident, index) => (
              <div key={incident.id} className="bg-white rounded-lg border border-gray-200 shadow-sm p-3">
                {/* Header: Status & CRM */}
                <div className="flex items-start justify-between mb-2 pb-2 border-b border-gray-100">
                  <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-md border ${getStatusColor(incident.status)}`}>
                    {getStatusText(incident.status)}
                  </span>
                  {incident.crmReferenceCode && (
                ...
                <div className="space-y-1.5 mb-2">
                  ...
                  <div className="flex items-start gap-1.5 text-xs">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="font-medium text-gray-900">{incident.handler?.fullName || 'Không xác định'}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-1.5 text-xs">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {incident.customer ? (
                        <span className="font-medium text-gray-900">{incident.customer.shortName}</span>
                      ) : (
                        <span className="font-medium text-gray-900">{incident.customerName || 'Không xác định'}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-1.5 text-xs">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="font-medium text-gray-900">{formatIncidentType(incident.incidentType)}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-1.5 text-xs">
                    ...
                    <div className="flex-1 min-w-0 space-y-0.5">
                      <div>
                        ...
                        <span className="text-green-600 font-medium">{formatDate(incident.startDate)}</span>
                      </div>
                      {incident.endDate && (
                        <div>
                          <span className="text-red-600">Kết thúc: </span>
                          <span className="text-red-600 font-medium">{formatDate(incident.endDate)}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {incident.notes && (
                    <div className="bg-green-50 border border-green-200 rounded px-2 py-1.5">
                      ...
                      <div className="text-xs text-green-700 line-clamp-2">{incident.notes}</div>
                    </div>
                  )}
                </div>
