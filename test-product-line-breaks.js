...
console.log('🧪 Testing Product Line Breaks');
console.log('==============================');
console.log('');

async function testProductLineBreaks() {
  try {
    console.log('📱 Testing product line breaks...');
...
🔗 <b>Xem chi tiết:</b> <a href="http://localhost:3001/admin/dashboard">Admin Dashboard</a>
    ...
    console.log('✅ Product line breaks test successful!');
    console.log(`📧 Message ID: ${response.data.result.message_id}`);
    console.log('📱 Check your Telegram app for the product line breaks!');
    
    return true;
  } catch (error) {
    console.log('❌ Product line breaks test failed:', error.message);
    return false;
  }
}
...
async function testFormattedProductList() {
  try {
    console.log('\n📱 Testing formatted product list with line breaks...');
...
🔗 <b>Xem chi tiết:</b> <a href="http://localhost:3001/admin/dashboard">Admin Dashboard</a>
    ...
    console.log('✅ Formatted product list with line breaks test successful!');
    console.log(`📧 Message ID: ${response.data.result.message_id}`);
    console.log('📱 Check your Telegram app for the formatted product list!');
    
    return true;
  } catch (error) {
    console.log('❌ Formatted product list with line breaks test failed:', error.message);
    return false;
  }
}
...
async function runProductLineBreaksTest() {
  try {
    console.log('🚀 Starting Product Line Breaks Test...');
    console.log('========================================');
    
    // Test 1: Raw product list with line breaks
    await testProductLineBreaks();
    
    // Test 2: Formatted product list with line breaks
    await testFormattedProductList();
    
    // Test 3: Server API
    await testServerAPI();
    
    console.log('\n🎉 PRODUCT LINE BREAKS TEST COMPLETED!');
    console.log('========================================');
    console.log('✅ Product line breaks are working!');
    console.log('✅ Formatted product list with line breaks is working!');
    console.log('✅ Server API is working!');
    console.log('');
    console.log('📱 Check your Telegram app for all messages');
    console.log('🔧 Each product is now displayed on a separate line!');
    console.log('');
    console.log('✨ Product Line Breaks Features:');
    console.log('• 🚨 Case NHẬN HÀNG được tạo');
    console.log('• 👨‍💼 Người xử lý: [Handler Name]');
    console.log('• 📋 Tiêu đề: [Case Title]');
    console.log('• 📝 Mô tả chi tiết: [Formatted Product List with Line Breaks]');
    console.log('• 📦 Nhận hàng từ cty: [Company Name]');
    console.log('• ⏰ Thời gian tạo: [Creation Time]');
    console.log('• 🔗 Xem chi tiết: Admin Dashboard');
    console.log('');
    console.log('📋 Product Format with Line Breaks:');
    console.log('• Tên sản phẩm | SL: 1 | Mã: ádfà | S/N: fsdfádf');
    console.log('• (Each product on a separate line)');
    console.log('');
    console.log('🚀 Next steps:');
    console.log('1. Create real receiving cases with product lists');
    console.log('2. Check Telegram for formatted product lists with line breaks');
    console.log('3. Verify each product is on a separate line');
    
  } catch (error) {
    console.log('\n❌ Product line breaks test failed:', error.message);
  }
}

// Run the product line breaks test
runProductLineBreaksTest();
