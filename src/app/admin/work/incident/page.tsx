...
export default function AdminIncidentWorkPage() {
  ...
  const filteredIncidents = incidents.filter(incident => {
    const matchesSearch = !debouncedSearchTerm ||
      ...
      incident.description.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
      incident.customerName.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
      incident.handler.fullName.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
      incident.incidentType.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
      (incident.customer?.fullCompanyName.toLowerCase().includes(debouncedSearchTerm.toLowerCase())) ||
      (incident.customer?.shortName.toLowerCase().includes(debouncedSearchTerm.toLowerCase()));
    ...
    const matchesDateFrom = !dateFrom || new Date(incident.startDate) >= dateFrom;
    const matchesDateTo = !dateTo || new Date(incident.startDate) <= dateTo;
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [debouncedSearchTerm, selectedHandler, selectedStatus, selectedIncidentType, selectedCustomer, dateFrom, dateTo]);
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      {(selectedHandler || selectedStatus || selectedIncidentType || selectedCustomer || dateFrom || dateTo) && (
                        <span className="absolute -top-1 -right-1 flex items-center justify-center w-4 h-4 text-[9px] font-bold text-white bg-red-500 rounded-full">
                          {[selectedHandler, selectedStatus, selectedIncidentType, selectedCustomer, dateFrom, dateTo].filter(Boolean).length}
                        </span>
                      )}
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {paginatedIncidents.map((incident, index) => (
                          <React.Fragment key={incident.id}>
                            <tr
                              className={`hover:bg-gray-50/50 transition-colors duration-150 cursor-pointer ${!isIncidentEvaluatedByAdmin(incident) ? 'bg-yellow-50/50 border-l-4 border-l-yellow-400' : ''
                                }`}
                              ...
                              <td className="px-2 py-4 whitespace-nowrap text-sm font-medium w-20">
                                <div className="flex items-center space-x-1">
                                  ...
                                  <button
                                    ...
                                    className={`p-1.5 rounded-md transition-colors duration-200 ${isIncidentEvaluatedByAdmin(incident)
                                ...
                                <td colSpan={10} className="px-3 py-6 bg-gray-50" onClick={(e) => e.stopPropagation()}>
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="md:col-span-2">
                                      ...
                                      <p className="text-xs text-gray-600 whitespace-pre-wrap leading-relaxed">{incident.notes || 'Không có ghi chú'}</p>
                                    </div>
                                  </div>
                                </td>
                              </tr>
